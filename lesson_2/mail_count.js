...
function parseData(data) {
  const messageDelimiter = /##\|\|##\s*/;
  return data.split(messageDelimiter);
}

function parseEmailContent(data) {
  const contentDelimiter = /#\/#\s*/;
  return data.split(contentDelimiter).reduce((obj, str) => {
    if ((/(.+): (.+)/).test(str)) {
...
function getSortedDates(data) {
  return data.map(email => email.Date)
             .sort((a, b) => (Date.parse(a) - Date.parse(b)));
}

let emailMessages = parseData(emailData).map(parseEmailContent);
let dates = getSortedDates(emailMessages);

console.log(emailMessages);
console.log(dates);
console.log(
  `There are ${emailMessages.length} email messages found in the string.\n` +
  `The emails are received between ${dates[0]} adnd ${dates.slice(-1)}.`
);