...
let positiveWords = ['fortune', 'dream', 'love', 'respect', 'patien', 'devout', 'noble', 'resolution'];
let negativeWords = ['die', 'heartache', 'death', 'despise', 'scorn', 'weary', 'trouble', 'oppress'];
let regexp = list => new RegExp(`\\b\\w*(${list.join('|')})\\w*\\b`, 'gi');

function analyze(sentimentScore) {
  if (sentimentScore > 0) {
    return 'positive';
...
function sentiment(text) {
  let positives = text.match(regexp(positiveWords));
  let negatives = text.match(regexp(negativeWords));
  let result = analyze(positives.length - negatives.length);

  return  `There are ${positives.length} positive words in the text.\n`+
